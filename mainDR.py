...
from Algorithm.SACDR import SACDR
from Network.ModelDR import YoloPretrainNet
...
agent = SACDR(env.observation_space, env.action_space, args)
yolo = YoloPretrainNet(torch.device(
...
for i_episode in itertools.count(1):
    ...
    while not done:
        ...
        next_state[0] = yolo.predict(next_state[0])
        episode_steps += 1
        total_numsteps += 1
    ...
    if i_episode % args.eval_per_steps == 0 and args.eval is True:
        ...
        for _ in range(episodes):
            ...
            while not done:
                ...
                next_state[0] = yolo.predict(next_state[0], True)
                episode_reward += reward
